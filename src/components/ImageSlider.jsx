			...
			<ul className={`relative flex ${width} ${height} overflow-hidden`}>
				{
					items.map((item, index) => (
						<li key={`${item.title}_${index}`} className={`absolute top-1/2 ${index === position ? 'z-0' : 'z-[-10]'} w-full -translate-y-2/4 -translate-x-2/4 duration-500`} style={{ left: `${(index - position) * 100 + 50}%` }}>
							<img className="w-full h-fit" src={item.url} alt={item.title} />
						</li>
					))
				}
			</ul>
